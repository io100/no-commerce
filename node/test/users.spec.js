  ...
  describe('POST /users', () => {
    ...
    it('should sign up', (done) => {
      request
        .post('/users')
        .set('Accept', 'application/json')
        .send({ user: { email: 'supercoolname', password: 'supersecretpassword' } })
        .expect(200, (err, res) => {
          if (err) { return done(err) }

          res.body.user.should.have.property('email')
          res.body.user.email.should.equal('supercoolname')
  ...
  describe('PUT /users/:id', () => {
    ...
    it('should update user', (done) => {
      ...
      request
        ...
        .send({ user: { email: 'updatedcoolname' } })
        .expect(200, (err, res) => {
          if (err) { return done(err) }
